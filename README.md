...
### One-command restart + verify (Codespaces/local)

From `backend/`, run (recommended):
...
If your clone does not yet have `scripts/`, use:
...
This script will:
- stop any old `node server.js` process
- start `npm start` in background
- verify `GET /api/health`
- verify hero stats text from homepage HTML

It uses `grep` (not `rg`) so it works in minimal shells too.

#### If `restart:dev` or `./restart-dev.sh` is missing

You are on an older checkout that does not include the latest scripts yet. Sync first:
...
If you still need a no-script fallback, run this directly from `backend/`:
...
## WittyDoctor Lifecycle Agent (Senior Dev + Solution Architect)

Use this repository-backed operating agent to manage full delivery lifecycle for website, webapp, and mobile:

- Agent charter: `docs/agents/wittydoctor-lifecycle-agent.md`
- Planning template: `docs/agents/wittydoctor-plan-template.md`
- Kickoff generator: `scripts/witty-agent-kickoff.sh`

Create a new initiative plan pack:

```bash
cd /workspaces/healthcare
./scripts/witty-agent-kickoff.sh <initiative-slug>
```

This generates plan + ADR + release checklist + risk register under `workplans/`.
